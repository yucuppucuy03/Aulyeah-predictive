...
try{var r=await fetch(url,{signal:AbortSignal.timeout(8000)});if(r.ok)return await r.json();}catch(e){}
for(var i=0;i<PROXIES.length;i++){
try{var r2=await fetch(PROXIES[i](url),{signal:AbortSignal.timeout(12000)});if(r2.ok)return await r2.json();}catch(e){}
}
...
if(buys===2)return”LEAN BUY”;
if(sells===2)return”LEAN SELL”;
return”MIXED”;
}
...
row.overall=getOverall(scores.filter(function(s){return s!==null;}));
return row;
}
...
if(!tfs.length){alert(“Pilih minimal 1 timeframe!”);return;}
...
btn.disabled=true;document.getElementById(‘csv-btn’).disabled=true;
...
allRows=[];activeFilter=‘all’;
document.querySelectorAll(’.fb’).forEach(function(b,i){if(i===0)b.classList.add(‘on’);else b.classList.remove(‘on’);});
try{
stxt.textContent=‘Fetching top ‘+topN+’ symbols…’;
var symbols=await fetchSymbols(topN);
document.getElementById(‘coin-count’).textContent=symbols.length;
buildHeader(tfs);
for(var i=0;i<symbols.length;i++){
var sym=symbols[i];
stxt.textContent=’[’ +(i+1)+’/’+symbols.length+’] ‘+sym.symbol+’ (’+tfs.join(’,’)+’)…’;
...
stxt.textContent=’Selesai! ‘+symbols.length+’ koin dianalisis pukul ’+now;
...
var tfs=[‘15m’,‘1h’,‘4h’].filter(function(tf){return document.getElementById(‘tf-’+tf).checked;});
var h=[’#’,‘Symbol’,‘Price’];
tfs.forEach(function(tf){h.push(tf+’_Bandar’,tf+’_Velocity’,tf+’_Dominasi’,tf+’*Struktur’,tf+’*Signal’);});
h.push(‘Overall’);
var lines=[h.join(’,’)];
...
a.download=’AulyeahMatrix*’+new Date().toISOString().slice(0,16).replace(‘T’,’*’)+’.csv’;
a.click();
}
</script>
