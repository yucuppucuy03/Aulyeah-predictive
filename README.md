...
.fb{font-family:'Share Tech Mono',monospace;font-size:9px;padding:4px 10px;border-radius:3px;border:1px solid var(--border);background:transparent;color:var(--dim);cursor:pointer;white-space:nowrap}
...
async function startScan(){
  ...
  try{
    ...
    let symbols = data.filter(d=>d.symbol.endsWith("USDT")).sort((a,b)=>b.quoteVolume-a.quoteVolume).slice(0,topN);
    ...
    for(let i=0; i<symbols.length; i++){
      let s = symbols[i];
      stxt.textContent=`[${i+1}/${symbols.length}] ${s.symbol}`;
...
function renderRow(r, idx, tfs){
  ...
  h+=`<td><span class="bdg ${r.overall.includes('BUY')?'bsb':r.overall.includes('SELL')?'bss':'bn'}">${r.overall}</span></td></tr>`;
  return h;
}
...
function exportCSV(){
  ...
  a.download=`Matrix_${new Date().getTime()}.csv`; a.click();
}
</script>
</body>
</html>
